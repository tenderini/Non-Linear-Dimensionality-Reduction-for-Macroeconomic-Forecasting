...
class Autoencoder(nn.Module):
    def __init__(self, n_features, hidden_layer_sizes, latent_size, decoder_type, device, load_params=True):
        ...
        self.device = device
        self.filename = "sourcecode/forecast_evaluation/forecast_algorithms/learned_parameters_D2FM/AE_input"+str(n_features)+"_"+decoder_type+"_".join([str(hidden_layer_sizes[i]) for i in range(len(hidden_layer_sizes))])+ ".pt"
...
class GRU(nn.Module):
    def __init__(self, n_features, hidden_size, num_layers, device, load_params=True):
        ...
        self.filename = "sourcecode/forecast_evaluation/forecast_algorithms/learned_parameters_D2FM/GRU_input"+str(n_features)+"_hidden"+str(hidden_size)+"_layers"+str(num_layers)+".pt"
        if load_params:
            try:
                self.load_state_dict(torch.load(self.filename))
...
class D2FM():
    ...
    def fit(self, d, batch_size, epochs, lr=0.001, schedule=0.999, autosave=True): 
        ...
        # Train static model
        print(f"----- Start fitting autoencoder -----", flush=True)
        ...
        # Train dynamic model
        print(f"----- Start fitting GRU -----", flush=True)
        ...
        # Save parameters in the "learned_parameters_D2FM" folder (it must be an existing folder)
        if autosave:
    ...
    def predict_next(self, horizon=1):
        ...
        for h in range(horizon) :
            ...
            last_sequence = torch.vstack([last_sequence[1:,:,:], predicted_factors.reshape((1,1,-1))])
...
class ForecastEvaluation():
    ...
    def evaluate(self, horizon=1):
        ...
        for t in range(self.hyper_params["test_size"]):
            ...
            idx_train = len(self.d) - self.hyper_params["test_size"] + t - horizon + 1   
            self.model.fit(
                d=self.d[:idx_train,:], 
            ...
            ## Predict next value
            predictions.append(self.model.predict_next(horizon=horizon))
...
class GridSearch():
    ...
    def grid_search(
        ...
        for i, hyper_param in enumerate(grid_list):
            ...
            if rmse < rmse_best:
                rmse_best = rmse
                with open('learned_parameters_D2FM/log_param.csv','a') as fd:
                    fd.write("\n"+str(hyper_param))
