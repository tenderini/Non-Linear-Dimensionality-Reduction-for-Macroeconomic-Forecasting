time_varying_pcr_penalized_ada_BIC <- function(d, target, horizon = 1, n_components=250,compute_components=TRUE) {
  ...
  lambda.opt.ADA=ADA.lasso.bic(x=as.matrix(d_pcr[,-(n_components+1)]),y=d_pcr[,(n_components+1)],penalty.factor)$lambda.opt 
  ...
  #y_hat <- cbind(1,as.matrix(d_test)) %*% rbind(model_LASSO.ADA$a0,model_LASSO.ADA$beta)*d_sd[target_idx] + d_mean[target_idx]
