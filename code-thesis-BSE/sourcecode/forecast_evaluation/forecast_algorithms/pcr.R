pcr_reg <- function(d, target, horizon = 1, n_eig=2, tune= TRUE, regularization = "none") {
  
  if(!(regularization %in% c("none","lasso", "adaptive_lasso", "ridge", "elastic_net","adaptive_lasso_BIC"))){
    stop("'regularization' must be one of c('none','lasso', 'adaptive_lasso', 'ridge', 'elastic_net','adaptive_lasso_BIC')")
  }
