isomap_reg <- function(d, target, horizon = 1) {
  ...
  d_pcr <- as.data.frame(cbind(
    # isomap$dim2[1:(nrow(isomap$dim2)-1),], y = y[2:length(y)], y_lag =  y[1:(length(y)-1)]  
    isomap$dim2[1:(nrow(isomap$dim2)-horizon),], y = y[(horizon+1):length(y)], y_lag =  y[1:(length(y)-horizon)]  
  ))
  
  d_test <- as.data.frame(cbind(isomap$dim2,y_lag=y))
